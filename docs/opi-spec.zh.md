...
本仓库已完成第三阶段：`opi` 二进制文件可用，多供应商流式处理可用，会话以 JSONL 持久化，压缩和 JSON 模式已存在，图像、上下文文件、Provider 加固和 TUI 日常使用基础已具备。第四阶段应优先收窄到可扩展性基底：RPC、SDK、扩展加载、资源发现、技能、提示片段、主题和包。
...
| 二进制 | `opi` 支持交互式 TUI、非交互文本模式、`--json`、会话命令、`--version` 和 `--help` |
...
| Crate | 当前表面 | 下一目标 |
|---|---|---|
| `opi-ai` | 供应商流式处理、模型注册表、用量/成本、重试/退避 | 企业供应商、图像输入/输出、共享 HTTP 客户端/代理加固 |
| `opi-agent` | 代理循环、钩子、队列、工具、会话、压缩 | 保持核心运行时狭窄；将协议适配器留给扩展表面 |
| `opi-agent` | `transport` 存根 | 保留给第 4 阶段 RPC/proxy，或在稳定 API 前移除 |
| `opi-tui` | ratatui 组件、markdown/代码、diff、主题、键绑定 | 图像渲染和模糊选择器 |
| `opi-coding-agent` | `clap` CLI、TOML 配置、内置工具、会话、JSON 模式 | pi 风格上下文文件、工具选择、shell 补全、供应商接线 |
| `opi-web-ui` | 带 `publish = false` 的占位 crate | 推迟的未发布 crate |
...
当前的 `transport` 存根保留给第 4 阶段 RPC/proxy 传输。在任何稳定公共 API 声明之前，它必须被隐藏为不稳定或被移除。
...
| 0.4.0 之后 | 第 4 阶段可扩展性 | GitHub + crates.io |
...
| # | 任务 | Crate |
|---|---|---|
| 4.1 | RPC JSONL 模式 | `opi-coding-agent` |
| 4.2 | SDK 嵌入表面 | `opi-coding-agent` / `opi-agent` |
| 4.3 | 扩展 trait 设计 | `opi-agent` |
| 4.4 | 扩展加载策略 | `opi-coding-agent` |
| 4.5 | 技能、提示片段、主题和包 | `opi-coding-agent` |
| 4.6 | 扩展示例：权限门禁、子代理、计划模式、待办、MCP 适配器 | 示例 / 包模板 |
| 4.7 | 会话分支 UI | `opi-agent` / `opi-tui` |
| 4.8 | 流式代理 | `opi-agent` 或新 crate |
| 4.9 | Web UI 实现 | `opi-web-ui` |

退出标准：第三方可以通过 RPC、SDK、扩展、技能、提示片段、主题和包组合并扩展 opi，而无需修补核心 crate。MCP、子代理、计划模式、待办和权限门禁应作为扩展或包演示，而不是核心功能。
