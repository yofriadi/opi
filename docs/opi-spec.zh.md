...
| 当前实现 | `opi` 0.4.0 workspace，第四阶段可扩展性基底已实现 |
| 下一里程碑 | extension 工作流和独立 Web 表面的产品化加固 |
...
本仓库已经在第三阶段终端编程 Agent 基础上完成第四阶段可扩展性基底：RPC JSONL 模式、共享 SDK 类型、extension hooks/tools/state、资源发现、skills、prompt fragments、themes、packages、自定义 provider/model 注册、会话分支选择、streaming proxy 原语，以及可复用的 Web-facing 组件/状态/渲染代码已经存在。MCP、子 Agent、plan mode、todos、permission gates、动态插件加载和独立浏览器应用应建立在该基底之上，而不是成为核心功能。
...
| 发布 CI | 六平台二进制工作流 |
| 可扩展性 | RPC JSONL、SDK 类型、extension API、资源/package 发现、自定义 provider/model registry、分支选择、streaming proxy，以及可复用 Web UI 组件/状态/渲染表面已经作为不稳定 0.x API 实现 |
...
| 0.4.0 workspace | 第 4 阶段可扩展性基底 | 可发布 crate 走 GitHub + crates.io；`opi-web-ui` 保持未发布 |
...
状态：当前 `0.4.0` workspace 中可扩展性基底已实现。

| # | 任务 | Crate |
|---|---|---|
| 4.1 | 带严格 framing、关联响应、异步事件，以及 session/model/thinking/compaction 命令的 RPC JSONL 模式 | `opi-coding-agent` |
