...
| 当前实现 | `opi` 0.5.2 workspace，第五阶段产品化 extension/package 生态系统已实现 |
...
opi 的会话格式是**Rust 原生**的仅追加 JSONL 树。它是一种独立格式，而非 pi 会话格式的副本：它只表示 pi 会话概念的*精选子集* —— 仅追加历史、基于父链接的分支、压缩摘要、模型与思考级别变更标记，以及持久化的扩展状态 —— 并基于 opi 的 Rust crate 实现。它**不**保证 pi session v3 的文件读写兼容性（见 9.4）。
...
opi 的会话 JSONL 是一种 Rust 原生格式，它表示 pi 会话概念的精选子集（仅追加历史、分支、压缩、模型与思考级别变更标记，以及扩展状态），但**不**保证 pi session v3 的文件兼容性。Opi 保留了 pi 的分支和压缩理念，但不使用其文件格式，因为 pi 存储 TypeScript 特定的扩展数据，opi 有独立的配置/插件计划，且意外的部分兼容性会产生误导。opi 刻意不沿用的概念包括：pi 的 TypeScript 特定扩展条目、其磁盘编码，以及任何“pi v3 会话文件可被 opi 打开、恢复或追加”的保证。未来的迁移命令可以将 pi v3 会话转换为 opi v1 会话，但在此之前两种格式不可互换。
...
| 0.5.1 workspace | 第 5 阶段产品化 extension/package 生态系统 | 可发布 crate 走 GitHub + crates.io；`opi-web-ui` 保持未发布 |
