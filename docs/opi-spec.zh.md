...
- `opi-web-ui`：未发布的未来 Web UI 占位符。

本仓库当前为 0.1.0 脚手架版本。工作区布局、CI、发布工作流、crate 边界和二进制名称已存在；功能实现尚未完成。第一阶段将这些占位符转化为一个实用的基于 Anthropic 的编程助手，包含六个工具、基础 TUI、TOML 配置和模拟供应商集成测试。
...
| 原则 | pi 0.75.3 | opi 设计 |
|---|---|---|
| 最小化核心 | `CONTRIBUTING.md` 规定核心之外的功能属于扩展 | 第 1-3 阶段避免插件/MCP/Web 范围蔓延 |
...
Opi 的 MVP 不是一个可扩展平台。MCP 是核心工具和权限稳定后的协议适配器；WASM 插件、子进程插件、RPC、多代理编排和 Web UI 是后续的扩展面。
...
| read/write/edit/bash/glob/grep 工具 | 第 1 阶段 | 行为对等 |
...
| 图像支持 | 第 3 阶段 | 语义对等 |
| 权限 | 第 3 阶段 | opi 用户体验 |
| MCP 客户端适配器 | 第 3 阶段 | 核心工具和权限之后的协议适配器 |
| 扩展/RPC/Web UI | 第 4 阶段 | opi 特定设计 |
...
| 内部依赖 | `opi-agent -> opi-ai`、`opi-web-ui -> opi-ai`、`opi-coding-agent -> opi-ai + opi-agent + opi-tui` |
| 外部依赖 | `tokio`、`serde`、`serde_json`、`thiserror`、`async-trait` |
| 二进制 | `opi` 支持 `--version` 和 `--help` |
...
当前 API 是占位符，可以在 0.2.0 之前被破坏。

| Crate | 当前占位符 | 目标 |
|---|---|---|
| `opi-ai` | `Provider::complete(&[Value]) -> Vec<StreamEvent>` | `Provider::stream(Request) -> EventStream` |
| `opi-ai` | 简单 `StreamEvent` 枚举 | start/delta/end/done/error 协议 |
| `opi-agent` | `Tool::execute(Value) -> Value` | schema 验证、取消、更新、执行模式 |
| `opi-agent` | `AgentState` 带原始值的结构体 | `Agent`、队列、状态枚举、订阅者 |
| `opi-agent` | `transport` 存根 | 保留给第 4 阶段或在稳定 API 前移除 |
| `opi-tui` | 空的 renderer/editor 存根 | ratatui 组件 |
| `opi-coding-agent` | 最小手写 CLI | `clap` CLI 和运行时连接 |
| `opi-web-ui` | 占位 `ChatWidget` | 推迟的未发布 crate |
...
| 异步 trait | `async-trait` | 已有，0.2.0 前移除 | 不是目标公共 API 依赖；dyn trait 使用显式的 boxed future/stream 返回值；内部非 dyn trait 可使用原生 async fn |
...
- `opi-coding-agent` 应当拥有编程特定的行为：内置文件工具、项目上下文、权限提示、CLI 配置和应用层会话命令。
...
当前的 `transport` 存根保留给第 4 阶段 RPC/代理传输。在 0.2.0 之前，它必须被隐藏为不稳定或被移除。
...
二进制拥有 CLI 解析、配置加载、供应商注册表构建、内置工具、系统提示构建、会话 UX、权限提示和运行时模式。
...
第 1 阶段必须包含高风险工具的最小安全边界。`write`、`edit` 和 `bash` 必须在执行前显示提议的路径或命令、有效的工作目录、环境策略、超时以及目标是否在工作区内。交互模式应当要求对这些高风险操作进行确认。非交互模式必须在运行变异文件工具或 shell 命令之前提供显式的选择加入策略。完整的可复用权限配置文件仍属于第 3 阶段范围。
...
2. 来自 `ToolDef` 的工具描述；
3. 用户系统提示文件；
4. `OPI.md` 项目上下文，从第 3 阶段开始；
5. 压缩摘要，从第 2 阶段开始；
6. 技能/提示片段，从第 4 阶段开始。

### 8.5 `opi-web-ui`

`opi-web-ui` 未发布且已推迟。实现时，它应消费与 JSON/RPC 模式相同的事件 schema，而不是依赖 TUI 内部实现。
...
交互模式是 stdin 为 TTY 时的默认模式。它拥有终端初始化、渲染、输入编辑、取消键和权限提示。
...
RPC 模式属于第 4 阶段。它应使用严格的 JSONL 帧：stdin 上每行一个命令，通过可选 `id` 关联响应，stdout 上发送异步事件。
...
- 路径遍历可以被允许，但权限策略应当能够限制它。
- 供应商 HTTP 必须默认使用 TLS。
- 第 1 阶段必须包含对 `write`、`edit` 和 `bash` 的最小确认和可审计性；更丰富的可复用权限配置文件属于第 3 阶段范围。
- 权限提示必须在高风险工具被视为生产就绪之前存在。

结构化参数降低了 shell 注入风险，但调用 shell 仍然执行模型提供的命令文本。缓解措施是权限、可审计性、超时、工作目录/环境控制和谨慎的命令构建。
...
| Bash 工具执行破坏性操作 | 高 | 高 | 顺序模式、权限、可见命令、超时 |
...
| 0.4.0+ | 第 3 阶段加固 | GitHub + crates.io |

首次 crates.io 发布由质量门控，而非仅由版本号决定。如果所有已发布的 crate 暴露真实的、文档化的行为而非占位公共 API，公共文档构建干净，合约测试覆盖已交付的供应商/工具/运行时边界，且发布技能的检查通过，它可以在 0.2.0 发生。如果这些门控未达标，crates.io 发布应当移至后续的 0.2.x 或 0.3.0 版本，同时 GitHub 二进制发布继续进行。因为二进制 crate 依赖内部库 crate，这些库应按依赖顺序一起发布；`opi-web-ui` 在有具体实现前保持未发布。所有 0.x 公共 API 除非另有明确文档说明，否则为不稳定。
...
| 3.6 | 终端图像渲染 | `opi-tui` |
| 3.7 | `OPI.md` 上下文加载 | `opi-coding-agent` |
| 3.8 | 可复用权限配置文件和策略系统 | `opi-coding-agent` |
| 3.9 | MCP 客户端适配器 | `opi-agent` |
...
退出标准：企业供应商可用，高风险工具有权限，发布产物可重复，交互式 UX 对日常使用足够健壮。

### 第 4 阶段 - 可扩展性

| # | 任务 | Crate |
|---|---|---|
| 4.1 | 扩展 trait 设计 | `opi-agent` |
| 4.2 | 扩展加载策略 | `opi-coding-agent` |
| 4.3 | 会话分支 UI | `opi-agent` / `opi-tui` |
| 4.4 | 多代理编排 | `opi-agent` |
| 4.5 | 技能和提示片段 | `opi-coding-agent` |
| 4.6 | Web UI 实现 | `opi-web-ui` |
| 4.7 | RPC JSONL 模式 | `opi-coding-agent` |
| 4.8 | WASM 或子进程插件运行时 | `opi-agent` 或接口审查后的新插件 crate |
| 4.9 | 流式代理 | `opi-agent` 或新 crate |

退出标准：第三方可以在不修补核心 crate 的情况下扩展 opi。
...
| ADR-015 | 插件策略 | 动态插件之前先做 MCP 适配器 | 协议集成风险低于任意运行时扩展 |
| ADR-016 | Web UI | 核心稳定前不发布 | 避免过早的 WASM 承诺 |
| ADR-017 | Transport 存根 | 保留给第 4 阶段或移除 | 避免未文档化的公共表面 |
...
架构不应排除 MCP 工具、远程工具执行、流式代理服务、编辑器集成、pi 会话迁移、插件运行时或 Web 聊天界面。这些不是 MVP 需求。
