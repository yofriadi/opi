...
| 当前实现 | `opi` 0.5.2 workspace，第 6 阶段对齐与可靠性加固已完成；第 5 阶段是 Rust 原生 package 和 process-adapter MVP |
| 下一里程碑 | 第 7 阶段可靠性/可观测性加固，随后是第 8 阶段运行时稳定化 |
...
本仓库已经在第三阶段终端编程 Agent 基础上完成第四阶段可扩展性基底：RPC JSONL 模式、共享 SDK 类型、extension hooks/tools/state、资源发现、skills、prompt fragments、themes、packages、自定义 provider/model 注册、会话分支选择、streaming proxy 原语，以及可复用的 Web-facing 组件/状态/渲染代码已经存在。第五阶段添加的是 Rust 原生 package 和 process-adapter MVP：本地和 git package 来源、`package add/remove/list/doctor` CLI、带 `[adapter]` 声明的 manifest V2、使用 `opi-extension-jsonl-v1` 协议的 `process-jsonl` adapter 托管，以及 adapter 到运行时的工具/命令/hooks/事件/状态/取消桥接。它不声称 pi package 生态对等，也不支持 npm package 安装、marketplace 行为、TypeScript extension live reload、通过 adapter 拦截 provider stream、自定义终端 UI adapter 渲染或 package 权限策略执行。MCP、子 Agent、plan mode、todos、permission gates、动态插件加载和独立浏览器应用应建立在该基底之上，而不是成为核心功能。
...
偏移账本至少应追踪 agent loop 语义、内置工具、会话格式、会话树语义、provider catalog、OAuth/subscription login、图像输入、图像生成、package 生态、TypeScript extension 兼容性、TUI renderer 架构、Web UI 产品对等性，以及 pi 保持在核心之外的工作流功能，例如 MCP、子 Agent、plan mode、todos、permission popups 和 background bash。
...
会话条目类型分为当前 v1 表面和第 11 阶段 v2 目标。第 11 阶段可以引入 `version = 2`，但必须保持 v1 文件可读，并且不得把自动迁移命令作为正常 resume 的前置条件。
...
| `session_info` | 第 11 阶段 v2 目标 | 会话名称和元数据 | 否 |
| `model_change` | 第 11 阶段 v2 目标 | 选择的供应商/模型已更改 | 否 |
| `thinking_level_change` | 第 11 阶段 v2 目标 | 思考级别已更改 | 否 |
| `label` | 第 11 阶段 v2 目标 | 用户标记或书签 | 否 |
| `branch_summary` | 第 11 阶段 v2 目标 | 用于 tree/context 重建的父分支摘要 | 是 |
| `custom_message` | 第 11 阶段 v2 目标 | 扩展提供的上下文消息 | 可配置 |
...
| 0.5.1 workspace | 第 5 阶段 Rust 原生 package 和 process-adapter MVP | 可发布 crate 走 GitHub + crates.io；`opi-web-ui` 保持未发布 |
| 0.5.2 workspace | 第 6 阶段文档对齐与可靠性加固 | 可发布 crate 走 GitHub + crates.io；`opi-web-ui` 保持未发布 |
...
状态：当前 `0.5.2` workspace 中可扩展性基底已实现。
