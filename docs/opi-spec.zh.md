...
| `Models` / provider-owned auth | 第 10 阶段 | `opi-ai` 中的 Rust 原生 provider collection/auth 缝合点，不承诺近期 OAuth 对等 |
...
非目标：OAuth login、subscription auth、广泛 provider catalog 扩张、图像生成、自定义 TUI extension protocol、npm/package marketplace、browser/web UI、`pi` TypeScript API 兼容、`pi` session file 兼容、共享 `opi-types` crate 或整体 loop 重写。
...
第十四阶段打磨内置终端产品：model/session/branch pickers、transcript rendering、command discovery、status/error feedback、accessibility、terminal compatibility，以及 image/diff presentation。它不承诺 custom extension UI、message renderer parity、web UI parity 或通用 TUI framework。
...
| npm/package gallery/update/enable/disable | Package adapter lifecycle、trust/source model、diagnostics 和 lock/update policy 稳定。 |
