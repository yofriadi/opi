...
| 当前实现 | `opi` 0.3.0，第二阶段已完成 |
| 下一里程碑 | 0.4.0 第三阶段 pi 对齐加固 |
...
本仓库已完成第二阶段：`opi` 二进制文件可用，多供应商流式处理可用，会话以 JSONL 持久化，压缩和 JSON 模式已存在，TUI 具备日常使用基础。第三阶段应加固 CLI 优先的编程代理体验，而不是把核心变成通用集成平台。
