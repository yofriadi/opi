...
| `@earendil-works/pi-coding-agent` | `opi-coding-agent` | Partial | CLI modes, built-in tools, config, sessions, context files, images, JSON/RPC, resources, packages, skills, prompt fragments, themes, custom provider registration, RPC extension commands, `/tree`, `/fork`, `/clone`, `--fork`, and same-file active-branch continuation via runtime `parent_id`/`leaf` entries are present. Product workflow breadth is still narrower than `pi`. | Prioritize package-manager workflow and external process/RPC package adapters. |
...
| `@earendil-works/pi-web-ui` | `opi-web-ui` | Deliberate Divergence | `opi-web-ui` is an unpublished Rust event/state/component/rendering crate. It is not the standalone browser component package that `pi-web-ui` provides. | Keep `publish = false` and document it as an RPC/SDK consumer surface until a separate web-app plan exists. |
...
| 4 | Extension hooks, tools, commands, messages, state | `opi-agent`, `opi-coding-agent` | `pi` TypeScript extensions | Partial | In-process Rust extension API and RPC/SDK command dispatch exist for embedders. | Add process/RPC adapters for external packages; do not load arbitrary Rust dylibs by default. |
...
| 4 | Packages | `opi-coding-agent` | `pi` packages and package manager | Partial | `package.toml` discovery and composition exist. | Add install/list/config/update/remove workflow if product parity is the target. |
...
| 4 | Web UI event/state/rendering | `opi-web-ui` | `pi-web-ui` browser package | Deliberate Divergence | Unpublished Rust consumer crate exists. | Keep claims narrow or create a separate browser app plan. |
...
|---:|---|---|---|
| P0 | Documentation truth | Version and phase status must match `Cargo.toml` and `CHANGELOG.md`. | Current docs describe the `0.5.0` workspace and keep historical `0.4.0` rows historical. |
...
| P1 | Extension/package execution | Packages can compose resources and registered extensions can route commands through RPC/SDK, but discovered packages still do not automatically become executable workflows. | External packages can expose process/RPC adapters without patching core. |
...
| P2 | Web UI scope | Current `opi-web-ui` is intentionally narrower than `pi-web-ui`. | Public docs avoid claiming a standalone browser app. |
...
- Update this matrix when a phase milestone, public extension surface, package workflow, provider family, session command, or web-ui claim changes.
