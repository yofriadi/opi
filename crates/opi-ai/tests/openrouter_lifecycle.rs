//! Contract tests for OpenRouter profile (OpenAiChatProvider::new_for_profile) --
//! real HTTP-level verification.
//!
//! Covers: SSE streaming, HTTP error mapping, no-terminal-event detection,
//! extra header verification.
//!
//! Uses wiremock to simulate the OpenRouter API endpoint without live API calls.
...
async fn stream_sends_extra_headers() {
    ...
    assert!(
        events.iter().any(|e| matches!(e, AssistantStreamEvent::Done { .. })),
