...
`Cargo.toml` 中的 workspace 包版本是 `0.6.0`。`opi` 既可以作为终端编程
...
当前工作树还包含未发布的 Phase 8 运行时稳定化工作：`opi-agent` 已文档化并用
守卫测试覆盖运行时事件顺序、hook/工具/取消语义、SDK/RPC 命令状态行为，以及公共
API 表面分层。除非 crate README 另有明确说明，wire protocol、extension/package
表面和 trace payload 都应视为不稳定 0.x。
